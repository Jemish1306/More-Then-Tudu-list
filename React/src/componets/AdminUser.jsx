...
import { addUser, editUser, deleteUser } from "../Redux/adminUserSlice";
...
const AdminUser = () => {
  ...
  //   axios
  //     .post("https://65003ffa18c34dee0cd496d7.mockapi.io/mock", inputdata)
  //     .then((respost) => {
  ...
  useEffect(() => {
    ...
    axios.get("http://localhost:3229/userget")
      .then((res) => {
        dispatch(setAdminuser(res.data));
        console.log('api gret data :>> ', res.data);
      })
      .catch((error) => {
        console.log("Error fetching dataddddd:", error);
      });
  }, [dispatch])
  ...
  //   return (dispatch)=>{
  //     axios.get( `localhost:3229/userget`)
  //     .then((res)=>{
  //       dispatch(setAdminuser(res.data))
  ...
  const hendalsubmit = (e, userId) => {
    ...
    if (isEditing) {
      ...
      dispatch(editUser({ id: inputdata.id, data: inputdata }));
      const updateadminUser = adminuser.map((user) =>
        user.id === inputdata.id ? inputdata : user,


         
        axios
          .put(`http://localhost:3229/edituser/${inputdata.id}`, inputdata)
          .then((res) => {
      ...
      axios
        .post("http://localhost:3229/user", inputdata)
        .then((res) => {
  ...
  const  hendaleditform = (userId) => {
    ...
    // dispatch(editUser({ id: userId.id, data: inputdata }));

    // const updateedAdminuser = adminuser.filter((user) => user.id === userId);

    // setAdminuser(updateedAdminuser);
  ...
  const handleDelete = (userId) => {
    ...
    axios
      .delete(`http://localhost:3229/deleteuser/${userId}`)
      .then((res) => {
        ...
        <div className=" h-full w-full items-center  flex flex-col">
          ...
          <div className="w-full  flex   p-4 justify-between">
            <div className="flex flex-col">
              <label className="text-xl font-semibold">Users list</label>
          ...
          {showForm ? (
            <div className="w-full h-screen flex items-center justify-center fixed inset-0 z-50">
              ...
              <form
                className="relative w-full max-w-md drop-shadow-lg items-center justify-center flex z-[51]"

              >
                <div className=" w-full h-[700px] border flex flex-wrap bg-white flex-col  shadow-lg rounded-lg  p-6 ">
                  <div className="flex pb-4 border-b justify-between  ">
                    <label className="text-lg font-semibold ">Add User </label>
                    <span
                  ...
                  <div className="flex-col flex  justify-center items-center  text-start pb-4 pt-6 ">
                    <label className="font-medium text-sm  w-96 pb-1">
                      Password
                    </label>
                    <input
                  ...
                  <div className=" flex flex-col">
                    <label className="font-medium text-sm   pb-1">Status</label>
                    <div className="flex ">
                      <Radio
